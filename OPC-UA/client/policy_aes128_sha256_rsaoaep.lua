...
local aes128Sha256RsaOaep
for _, endpoint in ipairs(resp.Endpoints) do
  if endpoint.SecurityPolicyUri == ua.SecurityPolicy.Aes128_Sha256_RsaOaep then
    aes128Sha256RsaOaep = endpoint
    break
  end
end

if not aes128Sha256RsaOaep then
  error("Cannot find aes128Sha256RsaOaep policy on the server")
end
...
resp, err = c:openSecureChannel(
  ...
  aes128Sha256RsaOaep.ServerCertificate)
