...
- `WebAuthnModule/.lua/webauthn.lua` - Generic server-side WebAuthn module.
- `WebAuthnModule/.preload` - Loads the module app.
- `example1/.preload` and `example1/index.html` - Introductory WebAuthn example.
- `example2/.preload`, `.login.lsp`, `.RegEmail`, `registered.shtml`, `index.html`, `help.lsp`, `logout.lsp` - More complete registration and login example.
- `mako.conf` - Selects the active example and holds supporting configuration.
- `WebAuthn.md` - Separate server-side API documentation.
...
3. **Reset the User Database**
   - After saving `mako.conf`, **delete the WebAuthn user database (`Webauthn.userdb`)** generated by Example 1.
