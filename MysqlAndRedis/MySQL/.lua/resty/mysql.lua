...
local cosocket
local function coRecon(self,opts,cb)
   ba.timer(function() self.cosocket=ba.socket.event(function(sock) cosocket(sock,self,opts,cb) end) end):set(3000,true)
end

cosocket=function(sock,self,opts,cb)
   local ok,err,errcode,sqlstate = self:connect(opts)
   local c=cb(ok,err,errcode,sqlstate)
   if not ok then
      if c then coRecon(self,opts,cb) end
      return
   end
   ...
   coRecon(self,opts,cb)
end

function _M:async(opts,cb)
   assert("table" == type(opts) and "function" == type(cb))
   if self.cosocket then self.cosocket:close() end
   self.sndQT,self.sndQHead,self.sndQTail,self.sndQElems={},1,1,0
   self.cosocket=ba.socket.event(function(sock) cosocket(sock,self,opts,cb) end)
   return true
end
...
function _M:execute(func)
   if func then
      ...
      self.sndQT[sndQHead]=func
      self.sndQHead = sndQHead+1
      self.sndQElems=self.sndQElems+1
      if "disabled" ==	self.cosocket:state() then self.cosocket:enable() end
   end
   return self.sndQElems
end
